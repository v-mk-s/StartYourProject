...
const std::string MySQLQuery::getFieldName(const unsigned int &field)
{
    if(field < 1)
    {
        //eror индекс должен быть больше 1
        return NULL;
    } else if(m_mFieldMap.size() < field){
        //eror индекс вышел за рамки
        return NULL;
    }
...
const std::string MySQLQuery::getString(const unsigned int &row, const unsigned int &field)
{
    ...
    {
        //eror запрос вернул 0 строк
        return NULL;
    } else if(GetResultRowCount() < row)
    {
        //eror выбрано больше строк, чем есть
        return NULL;
    } else if(row < 1)
    {
       //eror строка должна быть > 1
        return NULL;
    }
