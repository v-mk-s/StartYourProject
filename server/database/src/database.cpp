...
MainDataBase::MainDataBase(): 
    ...
    tags_data_table = std::make_unique<mysqlx::Table>(CreateTable(sqlconn, TEAM_TABLE, "(id int auto_increment, tag char(30) unique,PRIMARY KEY ( id ) )");
    user_tags_data = std::make_unique<mysqlx::Table>(CreateTable(sqlconn, TEAM_TABLE, "(user_name char(32) not null, id_tag int not null, foreign key ( user_name ) references user_data( user_name ),foreign key ( id_tag ) references tags_data( id ), unique(user_name,id_tag))");
    project_tags_data = std::make_unique<mysqlx::Table>(CreateTable(sqlconn, TEAM_TABLE, "(project_name char(32) not null, id_tag int not null, foreign key ( project_name ) references project_data( project_name ),foreign key ( id_tag ) references tags_data( id ), unique(project_name,id_tag))");
    // db = std::make_unique<mysqlx::Schema>(sqlconn.getSchema("SYP_DB"));
